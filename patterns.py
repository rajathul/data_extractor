def reg_patterns():
    patterns = {
        ...
        "montante e moeda": r"montante e moeda(?:\s*do\s*(?:empréstimo\s*a\s*conceder)?)?:?\s*([\d\s\.,]+)\s*([a-zA-Z]{3}|€)",
        ...
        "mtic (base)": r"(?:(?:mtic \(base\)\s*€\s*)|(?:montante total a\s*(?:reembolsar\s*)?(mtic):))([\d\s\.,]+)",
